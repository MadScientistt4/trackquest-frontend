...
const CandleChart = ({ candles, width = 800, height = 400 }: { candles: CandleData[]; width?: number; height?: number }) => {
  useEffect(() => {
    ...
    const canvas = document.getElementById('crypto-chart-canvas') as HTMLCanvasElement
    if (!canvas) return
  ...
  return <canvas id="crypto-chart-canvas" width={width} height={height} className="w-full border border-gray-700 rounded-lg" />
}
...
export default function CryptoPredictionPage() {
  ...
  const loadCryptos = async () => {
    try {
      const res = await gamesApi.getAvailableCryptos()
      setCryptos(res.data.cryptos)
    } catch (error) {
      toast.error('Failed to load cryptos')
    }
  }
  ...
  const startGame = async () => {
    try {
      ...
      const res = await gamesApi.getCryptoChallenge(selectedCrypto, difficulty)
  ...
  const submitPrediction = async () => {
    ...
    try {
      ...
      const res = await gamesApi.submitCryptoPrediction({
        prediction,
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-4">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2">🪙 Crypto Prediction</h1>
          <p className="text-gray-400">Master volatile crypto markets with real data from Polygon.io</p>
        </div>

        {gameState === 'loading' && (
          <div className="space-y-6">
            <div className="bg-gray-800 p-6 rounded-lg">
              <label className="block text-sm font-semibold mb-3">Select Cryptocurrency</label>
              <select
              ...
              >
                {cryptos.map((crypto) => (
                  <option key={crypto.symbol} value={crypto.symbol}>
                    {crypto.symbol} - {crypto.name} ({crypto.pair})
                  </option>
                ))}
              </select>
            </div>
                ...
                {['easy', 'medium', 'hard'].map((level) => (
                  <button
                    ...
                    className={`py-3 px-4 rounded-lg font-semibold transition ${
                      difficulty === level ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                    }`}
                  >
            ...
            <button
              onClick={startGame}
              className="w-full bg-purple-600 hover:bg-purple-700 py-3 px-4 rounded-lg font-bold text-lg transition"
            >
              Start Game
            </button>
          </div>
        ...
        {gameState === 'playing' && (
          <div className="space-y-6">
            ...
            <button
              ...
              className="w-full bg-purple-600 hover:bg-purple-700 disabled:bg-gray-600 py-3 px-4 rounded-lg font-bold text-lg transition"
            >
